...
官网：<https://timetable.polaris-io.ccwu.cc/>
...
> **隐私承诺**：Polaris 默认开启每天最多一次的自动版本检查，仅访问官方 GitHub / Gitee 发布源读取公开版本清单；只有你手动选择下载更新时才会获取安装包（详见 [PRIVACY.md](PRIVACY.md)）。课程、课表、学校、账户资料、设置及导入文件不会上传。
...
- **一键备份/恢复**全部课表、计划、考试/DDL等事件与设置（`.polarisbackup` 文件）；背景图和头像仅保存图片 URI

### 应用内更新
- 「更多 → 更新 → 检查更新」手动检查稳定版更新；自动检查更新默认开启，每天最多一次，可随时关闭
