...
自主迭代交付 4 个版本（1.17.0 / 1.17.1 / 1.18.0 / 1.18.1），全部通过 单测 275+6 全绿、instrumented 12 条全绿、模拟器明暗双主题截图验收：
...
后续候选（未排期）：英文本地化（653 条 strings 的机械轮）、考试/DDL 实体（需新数据模型，建议用户决策）、widget 进行中高亮。
