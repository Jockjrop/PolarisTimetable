...
- ✅ **4-2 pdfbox-android 评估结论（不升版，护栏落地）**：`com.tom-roush:pdfbox-android:2.0.27.0` 已是 Android 移植线最新版（上游 Apache 2.0.x 已至 2.0.37，但 TomRoush 移植 2023 年后停更，无 3.x 移植），**无版本可升**。已交付三校 PDF 回归护栏 `androidTest/.../parser/PdfRegressionTest.java`：样例 PDF 因隐私不入库，经 `adb push` 至 app 内部 `files/pdfregression/` 后本地运行，缺文件自动跳过（Assume），CI 不受影响。基线（pdfbox 2.0.27.0，模拟器 API 35 实测，样例以 `test pdf/` 校名映射）：西邮×XUPT=11 课 0 错误、西理工×XAUT=24 课 2 条非致命诊断、杭电×HDU=27 课 0 错误——三校各自模型均解析成功。未来升级 pdfbox 或改解析器时以此护栏对比漂移。

风险：`pdfbox-android` 是解析核心，跨版本升级可能改变文本坐标提取行为 —— 必须一校一测，不与其他改动混在同一轮。

### 阶段 5：资产与工具链入库（P5，不升版本）✅ 已完成

- ✅ `build/` 下 10 个 `.py` 脚本已迁入 `tools/` 目录并入库（本地化与资源生成工具）。
- ✅ `polaris-ui-design/` 已入库（11 文件 0.1MB 纯文本，体积可控；同步从 `.gitignore` 摘除该条目，`polaris-ui-design-draft/` 仍保持忽略）。
- ✅ `preview.html` 已移入 `docs/archive/`（git 识别为 rename，历史保留），根目录只保留活跃资产。

### 收官后修复：壳层毛玻璃「快速下滑透视」序列（1.16.1 → 1.16.4）✅ 已收敛

阶段 5 竣工后，用户实测持续报告「快速下滑时壳层（顶栏/底栏）变透视」。四轮修复与最终定位：

| 版本 | 动作 | 结果 |
|---|---|---|
| 1.16.1 | 恢复实时采样（子树滚动签名） | 采样冻结已修，色带仍在 |
| 1.16.2 | 加深蒙层 + 硬件层保障 RenderEffect | 静态可接受，快滑仍透视 |
| 1.16.3 | 壳层高斯半径 22→28dp | 改善不足 |
| 1.16.4 | 壳层高斯半径 28→60dp | ✅ 快滑/静止/滑入动画三态均磨砂 |

**1.16.4 定位结论（模拟器 API 35 注入 11 门课程 + adb 快滑抓帧实测）**：
采样与 RenderEffect 链路本身健康——preDraw 子树签名与内容同帧重绘（无滞后）、
采样缓冲可见区 alpha 255 不透明、半径 A/B（28 vs 60）画面有差异证明高斯在
gfxstream 上生效。真正根因是**模糊量不足**：backdrop 缓冲按 0.24 降采样后，课块
（约 40px 宽）在玻璃下成为约 10px 宽的色带，高饱和课块色透过 60% 蒙层仍保留
色带分界，28dp 高斯抹不平，快滑时色带扫过玻璃即被感知为「透视」。60dp 实测将
色带糊为均匀磨砂。临时诊断代码与一次性种子测试已随修复移除。

排查中确认的既有行为（非缺陷）：快速下滑触发 `hideBottomNav()` 底栏下滑隐藏动画，
玻璃半没入屏幕属预期交互；隐藏位下 backdrop 缓冲超出屏幕的底部行透明为正常现象。
验证技巧：半径 A/B 对比必须控制同一滚动位置（重启应用回顶部），否则背景内容不同
不可比；`screencap` 为无损帧，录屏 mp4 有压缩噪声且 VFR 帧稀疏，定量验收用前者。
...
| 4 | minor（1.16.0） |
| 收官后毛玻璃修复 | patch 递增，1.16.1 → 1.16.4 |
